class Api::V1::NaoConformidadesController < Api::V1Controller
	...
	def index
		...
		@nao_conformidades = NaoConformidade.where("(status = ? AND usuario_id = ?) OR (status = ? AND usuario_delegado_id =?)",NaoConformidade::CRIADO,@usuario_id,NaoConformidade::ESCALONADO,@usuario_id).order("data DESC")
